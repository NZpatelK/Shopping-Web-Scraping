...
while (page <= max_page):
    ...
    plist = requests.get('https://kensmart.co.nz/products.json?limit=1000&page='+str(page)).json()
    for product in plist['products']:

        try:
            newProduct = {}
            newProduct['businessName'] = 'Ken Mart'
...
with open('../AsiaGrocorData.json', 'w', encoding='utf-8') as f:
