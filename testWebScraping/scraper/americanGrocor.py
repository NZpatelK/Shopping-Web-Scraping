...
while (page <= max_page):
    ...
    plist = requests.get('https://www.internationalfoods.co.nz/products.json?limit=1000&page='+str(page)).json()
    for product in plist['products']:

        try:
            newProduct = {}
            newProduct['businessName'] = 'International Foods'
...
with open('../AmericanGrocorData.json', 'w', encoding='utf-8') as f:
